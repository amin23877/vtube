...
import ShadowBox from "@/components/header/ShadowBox";
import pageImage from "@/assets/page-bg.png";
...
export default function AppBar({ children }: { children: ReactNode }) {
  ...
  return (
    <>
      <div
        className={`transition-all duration-200 sticky top-0 z-[1] m-10 mb-0 bg-gray-400 bg-opacity-5 shadowBox ${
          mode ? "" : "rounded-t-3xl"
        }`}>
        <div
          className={`flex items-center ${
            mode ? "justify-evenly" : "justify-end"
          }`}>
          <div
            onClick={() => router.push("/")}
        ...
        <div
          className={`transition-all duration-200 flex items-center gap-6 flex-col ${
            display ? "hidden" : ""
          } ${mode ? "opacity-0 translate-y-[-20px] h-0" : "h-[170px]"}`}>
      ...
      <ShadowBox src={pathname === "/" ? pageImage : ""}>{children}</ShadowBox>
    </>
  );
}
